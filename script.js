...
button.addEventListener("click", () => {
    ...
    const typing = setInterval(() => {
        ...
        if(index >= dream.length){
            ...
            output.innerHTML = " Your dream link is generated below!";
            const dreamWords = dream.trim().split(" ");
            const mainKeyword = encodeURIComponent(dreamWords[dreamWords.length - 1]);
            const randomLock = Math.floor(Math.random() * 10000);
            const siteUrl = `https://loremflickr.com/1200/800/${mainKeyword}?lock=${randomLock}`;
            generatorLink.href = siteUrl;
            generatorLink.style.display = "inline-block";
        }
    }, 40);
